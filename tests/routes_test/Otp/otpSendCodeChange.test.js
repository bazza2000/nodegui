...
import OtpSendCodeChange from "../../../src/routes/OTP/OtpSendCodeChange";
...
describe('Header component', () => {
  ...
  it('should render at least a div', () => {
    const shallowWrapper = shallow(<OtpSendCodeChange  />, {context}).dive({context});
  ...
  it('should render the state as expected', () => {
    const shallowWrapper = shallow(<OtpSendCodeChange />, {context}).dive({context});
    const expectedState = { currentlySending: false, generalError: null, message: null , isVerifyScreen:true};
  ...
  it('should check send code function', () => {
    const shallowWrapper = shallow(<OtpSendCodeChange />, {context}).dive({context});
    let event = {keyCode:13};
    shallowWrapper.instance()._sendCode(event);
  });
});